...
router.post('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error scheduling waste collection: ' + error.message });
    }
});
...
router.put('/:id', async (req, res) => {
    try {
        const { date, time } = req.body;
        const { id } = req.params;
        const updated = await WasteCollection.update({ date, time }, { where: { id, userId: req.user.id } });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error cancelling schedule: ' + error.message });
    }
});
